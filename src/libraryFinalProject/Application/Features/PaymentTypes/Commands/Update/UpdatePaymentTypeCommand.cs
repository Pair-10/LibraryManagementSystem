...
public class UpdatePaymentTypeCommand : IRequest<UpdatedPaymentTypeResponse>, ISecuredRequest, ICacheRemoverRequest, ILoggableRequest, ITransactionalRequest
{
    public Guid Id { get; set; }
    public string Name { get; set; }

    public string[] Roles => [Admin, Write, PaymentTypesOperationClaims.Update];

    public bool BypassCache { get; }
    public string? CacheKey { get; }
    public string[]? CacheGroupKey => ["GetPaymentTypes"];

    public class UpdatePaymentTypeCommandHandler : IRequestHandler<UpdatePaymentTypeCommand, UpdatedPaymentTypeResponse>
    {
        private readonly IMapper _mapper;
        private readonly IPaymentTypeRepository _paymentTypeRepository;
        private readonly PaymentTypeBusinessRules _paymentTypeBusinessRules;

        public UpdatePaymentTypeCommandHandler(IMapper mapper, IPaymentTypeRepository paymentTypeRepository,
        ...
        public async Task<UpdatedPaymentTypeResponse> Handle(UpdatePaymentTypeCommand request, CancellationToken cancellationToken)
        {
            ...
            paymentType = _mapper.Map(request, paymentType);

            await _paymentTypeRepository.UpdateAsync(paymentType!);

            UpdatedPaymentTypeResponse response = _mapper.Map<UpdatedPaymentTypeResponse>(paymentType);
            return response;
        }
    }
}